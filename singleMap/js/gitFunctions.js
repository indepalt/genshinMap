...
function upLoadSaveData(localSave) {
  if (tokenPara !== undefined && tokenPara !== "") {
    var fileName = window.prompt("기록 이름을 입력해세요.");
    if (fileName) {
      //console.log(fileName);
      checkFile(fileName);
    }
  } else {
    if (window.confirm("클라우드 저장을 하려면 로그인 해야합니다. 지금 로그인할까요?")) {
      jumpLogin();
    } else {
      //调用本地存档
      localSave();
...
function getFileList() {
  ...
  var success = function (res) {
    if (res.data === "login") {
      alert("등록되지 않은 gitee 계정 혹은 만료된 계정입니다.");
    } else {
      //console.log(res);
  ...
  if (tokenPara != null) {
    ...
    var err = function (res) {
      ...
      if (res.status == 401 && res.responseJSON.message == "401 Unauthorized: Application has exceeded the rate limit") {
        var msg = "클라이언트 접속 수가 초과됐습니다. 나중에 다시 시도해주세요.";
        if (confirm(msg) == true) {
          ...
          if (currentClients.length > 1) {
            ...
            alert("클라이언트 접속수가 초과됐습니다.");
          }
        }
      } else if (res.status == 401) {
        var msg = "로그인이 유효하지 않습니다. 다시 로그인 하려면 확인을 눌러주세요.";
        if (confirm(msg) == true) {
          jumpLogin();
        }
      }
    ...
    var success = function (res) {
      ...
      if (IDSync && !TimeSync && confirmSync) {
        confirmSync = false;
        var msg = "기록과 클라우드 저장 시간이 일치하지 않는 것이 감지되었습니다. 동기화 하시겠습니까?(클릭시 동기화)";
        if (confirm(msg) == true) {
          loadGistFile(currentID, tempLastUpdateTime);
          confirmSync = true;
        } else {
          var msg = "현재 기록된 파일을 클라우드에 업로드 하시겠습니까? (클릭시 업로드)";
          if (confirm(msg) == true) {
            confirmSync = true;
  ...
  if (tokenPara != null) {
      ...
      if (res.status == 401 && res.responseJSON.message == "401 Unauthorized: Application has exceeded the rate limit") {
        ...
        if (confirm(msg) == true) {
          ...
          if (currentClients.length > 1) {
            ...
            alert("클라이언트 접속 수가 초과됐습니다.");
          }
        }
      } else if (res.status == 401) {
        var msg = "로그인이 유효하지 않습니다. 다시 로그인하려면 확인을 눌러주세요.";
        if (confirm(msg) == true) {
          jumpLogin();
        }
      }
    ...
    var success = function (res) {
      ...
      if (isSync && (localStorage.getItem("NetSync") == "false")) {
        ...
        $(".loading").hide();
        alert("현재 기록이 이미 최신입니다.");
      } else {
        $(".loading").hide();
        console.log("동기화할 기록을 선택하지 않아 자동 동기화에 실패했습니다.");
      }
    };
    $.ajax({
      async: false,
...
function checkAutoUpdate() {
  if (localStorage.getItem("NetSync") == "false") {
    ...
    console.log("기록에 변화가 없어 자동 동기화 하지 않음");
  }
}
...
function addGistFile(_default = '', cb) {
  ...
  var description = window.prompt("기록 이름을 입력하세요", _default);
  if (description) {
    var success = function (res) {
      if (isSync == false) {
        confirmSync = true;
      ...
      alert(isCopy ? '복사 완료' : '신설 완료');
      cb && cb(res)
      getGistList();
    };
    var err = function (res) {
      //console.log(res.responseJSON.message);
      if (res.responseJSON.message == "인증이 안됐으니 전화번호 등록후 이용 바랍니다.") {
        if (window.confirm("클라우드 기록을 위해 휴대폰을 연동해야 합니다. 연동하시겠습니까?")) {
          BDSJ();
        }
...
function updateGistDescription(fileID) {
  var description = window.prompt("(기록 이름 수정) 기록 이름을 입력해주세요");
  if (description) {
    var success = function (res) {
      //console.log(res);
...
function deleteGistFile(fileID) {
  var msg = "기록 삭제후 복구가 불가능 합니다. 삭제하시겠습니까?";
  if (confirm(msg) == true) {
    var success = function (res) {
      //console.log(res);
      alert("삭제완료！");
...
function loadGistFile(fileID, fileLastUpdateTime) {
  ...
  if (isSync == false && markersData.length >= 1 && confirmSync == true) {
    var msg = "동기화 되지 않았습니다. 분실을 방지하기 위해 신규 기록을 만드려면 클릭하세요.";
    if (confirm(msg) == true) {
      addGistFile();
    }
  } else {
